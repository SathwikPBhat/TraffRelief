...
function AuditSummary() {
  ...
  const { auditId } = useParams();         // URL param (e.g. AUD-101)
  const navigate = useNavigate();

  const codeId = auditId;                  // Display code
  ...
  useEffect(() => {
    if (!auditId || !token) {
      setErr('Missing audit code or auth token');
      setLoading(false);
      return;
    }
    (async () => {
      try {
        // Endpoint should return one audit by its auditId code
        // Implement in backend: GET /audit/by-code/:auditId
        const res = await fetch(`http://localhost:5000/admin/audit-summary/${auditId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || data.error || 'Failed to fetch audit');
        setAudit(data);
      } catch (e) {
        setErr(e.message);
      } finally {
  ...
  const fieldMap = useMemo(() => ([
    ...
    ['Medical Conditions Requiring Treatment', 'medicalConditionsRequiringTreatment'],
      ...
      <div className="w-full rounded-xl border border-green-600 bg-green-50 text-green-800 p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {rows.map(([label, status]) => (
            <div key={label} className="rounded-lg border border-green-300 bg-white text-green-700 px-3 py-2">
        ...
        <div className="mt-3 text-sm">
          ...
          {r.next_audit_date && (
            <span className="ml-4">
              Next Audit: <span className="font-semibold">{displayDate(r.next_audit_date)}</span>
            </span>
          )}
        </div>
    ...
    <main className="min-h-screen w-full font-['QuickSand'] flex flex-col bg-stone-100">
      ...
      <div className="w-full p-6 md:p-10">
        <div className="flex items-start justify-between mb-6">
          <div>
            <div className="text-teal-700 text-2xl font-semibold">Audit Summary</div>
            <div className="mt-1 text-slate-600 text-sm">
              {codeId && (
                <span className="mr-3">
                  Code: <span className="font-medium text-slate-800">{codeId}</span>
                </span>
              )}
              {audit?.timestamp && (
                <span className="mr-3">
                  Date: <span className="font-medium text-slate-800">{displayDate(audit.timestamp)}</span>
                </span>
              )}
            </div>
          </div>
