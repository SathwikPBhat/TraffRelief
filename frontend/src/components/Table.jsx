...
function Table({ tableHeaders, tableData, about }) {
  const navigate = useNavigate();
  const handleNavigate = (id) => {
    if (about === "victim") navigate(`/victim-profile/${id}`);
    else toast.error("Navigation not defined", { toastId: "navError" });
  };
  return (
        ...
        <tbody className="bg-white font-['QuickSand'] font-medium">
          {tableData.map((val, i) => {
            return (
              ...
              >
                <td className="py-3 px-4 text-sm text-slate-900">{val.name}</td>
